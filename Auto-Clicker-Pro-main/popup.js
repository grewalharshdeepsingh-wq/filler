...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Time Delay Changer Controls
    const globalClickDelay = document.getElementById('globalClickDelay');
    const applyDelayToAllBtn = document.getElementById('applyDelayToAllBtn');
    ...
    function updateActivePreset(val) {
        delayPresetBtns.forEach(btn => {
            const btnVal = parseInt(btn.dataset.delay);
    ...
    if (globalClickDelay) {
        ...
        globalClickDelay.addEventListener('input', () => {
            const val = parseInt(globalClickDelay.value);
            if (!isNaN(val)) updateActivePreset(val);
        });
    }
    ...
    if (addBeforePageBtn) {
        addBeforePageBtn.addEventListener('click', () => {
            const currentDelay = parseInt(globalClickDelay ? globalClickDelay.value : 1000) || 1000;
            chrome.storage.local.get(['steps'], (data) => {
                const steps = data.steps || [];
                steps.push({
                    action: 'navigateBack',
                    title: 'Move to Before Page',
                    delay: Math.max(1000, currentDelay)
                });
                chrome.storage.local.set({ steps });
            });
        });
    ...
    chrome.storage.onChanged.addListener((changes, area) => {
        if (area === 'local') {
            chrome.storage.local.get(['isRunning', 'isRecording', 'steps', 'smartMode', 'paragraphs', 'defaultDelay'], (data) => {
                ...
                if (changes.defaultDelay) {
                    const delay = parseInt(changes.defaultDelay.newValue) || 1000;
                    if (globalClickDelay) globalClickDelay.value = delay;
                    updateActivePreset(delay);
                }
            });
        }
    });
    ...
    function initialize() {
        chrome.storage.local.get([
            'steps', 'isRunning', 'isRecording', 'loopSettings', 'smartMode', 'paragraphs', 'defaultDelay'
        ], (data) => {
            renderSteps(data.steps || []);
            updateUI(data.isRunning || false, data.isRecording || false);
            updateParagraphUI(data.paragraphs || []);

            const initialDelay = parseInt(data.defaultDelay) || 1000;
            if (globalClickDelay) globalClickDelay.value = initialDelay;
            updateActivePreset(initialDelay);
