...
let currentRecordingDelay = 1000;
...
// Initialize on load
const init = () => {
    chrome.storage.local.get(['isRecording', 'smartMode', 'steps', 'isRunning', 'runningState', 'loopSettings', 'defaultDelay'], (data) => {
        smartModeCached = data.smartMode !== undefined ? data.smartMode : true;
        recordedSteps = data.steps || [];
        currentRecordingDelay = (data.defaultDelay !== undefined && parseInt(data.defaultDelay) >= 0) ? parseInt(data.defaultDelay) : 1000;
        if (data.isRecording) {
...
chrome.storage.onChanged.addListener((changes, area) => {
    if (area === 'local') {
        ...
        if (changes.defaultDelay !== undefined) {
            currentRecordingDelay = parseInt(changes.defaultDelay.newValue) >= 0 ? parseInt(changes.defaultDelay.newValue) : 1000;
            const bannerDelayInput = document.getElementById('acp-banner-delay-input');
            if (bannerDelayInput && document.activeElement !== bannerDelayInput) {
                bannerDelayInput.value = currentRecordingDelay;
            }
        }
    }
});
...
const showRecordingBanner = (branch = null) => {
    ...
    banner.innerHTML = `
        ...
        <div class="acp-banner-text">
            ...
            <div style="display:flex; align-items:center; gap:5px; margin-top:3px; font-size:11px; color:#cbd5e1;">
                <span>Delay:</span>
                <input type="number" id="acp-banner-delay-input" min="0" max="60000" step="100" value="${currentRecordingDelay}" style="width:58px; padding:2px 4px; font-size:11px; background:#0f172a; color:#38bdf8; border:1px solid #334155; border-radius:4px; text-align:center;">
                <span>ms</span>
                <button id="acp-banner-apply-all-btn" class="acp-field-pill-btn" style="font-size:10px; color:#a78bfa; padding:2px 6px;" title="Apply this delay to all existing steps">Apply All</button>
            </div>
            ${branchBadge}
            ${toolControls}
    ...
    const bannerDelayInput = document.getElementById('acp-banner-delay-input');
    if (bannerDelayInput) {
        bannerDelayInput.onchange = (e) => {
            e.stopPropagation();
            const val = Math.max(0, parseInt(e.target.value) >= 0 ? parseInt(e.target.value) : 1000);
            currentRecordingDelay = val;
            chrome.storage.local.set({ defaultDelay: val });
            showBannerToast(`Default delay: ${val}ms`);
        };
        bannerDelayInput.onkeydown = (e) => { e.stopPropagation(); };
    }

    const bannerApplyAllBtn = document.getElementById('acp-banner-apply-all-btn');
    if (bannerApplyAllBtn) {
        bannerApplyAllBtn.onclick = (e) => {
            e.stopPropagation();
            const inputVal = bannerDelayInput ? parseInt(bannerDelayInput.value) : currentRecordingDelay;
            const val = Math.max(0, !isNaN(inputVal) ? inputVal : currentRecordingDelay);
            currentRecordingDelay = val;
            recordedSteps = recordedSteps.map(step => ({ ...step, delay: val }));
            saveSteps();
            chrome.storage.local.set({ defaultDelay: val });
            showBannerToast(`✓ Applied ${val}ms to all ${recordedSteps.length} steps`);
        };
    }
        ...
        if (navBackBtn) {
            navBackBtn.onclick = (e) => {
                ...
                const step = {
                    ...
                    delay: Math.max(currentRecordingDelay, 1000)
                };
                recordedSteps.push(step);
                saveSteps();
...
const executeStep = async (step) => {
    ...
    if (step.action === 'condition') {
        ...
        for (const subStep of branchSteps) {
            ...
            const subDelay = (subStep.delay !== undefined && !isNaN(Number(subStep.delay))) ? Math.max(0, Number(subStep.delay)) : 500;
...
const executeSteps = async (steps, loop, startStepIndex = 0, startLoop = 1) => {
    ...
    while (currentLoop < loopCount && !stopExecution) {
        ...
        for (let i = initialI; i < steps.length; i++) {
            ...
            const stepDelay = (steps[i].delay !== undefined && !isNaN(Number(steps[i].delay))) ? Math.max(0, Number(steps[i].delay)) : 1000;
